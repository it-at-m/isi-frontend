...
describe("DokumenteTest.spec.ts", () => {
  ...
  test('Maximum Anzahl von Dokumenten erreicht', () => {
    ...
    expect(maxNumberOfFilesReached([] as DokumentDto[], mockFileList, { maxNumberOfFiles: undefined } as FileInformationDto)).toBe(false);
    expect(maxNumberOfFilesReached([] as DokumentDto[], mockFileList, { maxNumberOfFiles: 3 } as FileInformationDto)).toBe(false);
    expect(maxNumberOfFilesReached([ createDokumentDto() ] as DokumentDto[], mockFileList, { maxNumberOfFiles: 3 } as FileInformationDto)).toBe(false);
    expect(maxNumberOfFilesReached([ createDokumentDto(), createDokumentDto() ] as DokumentDto[], mockFileList, { maxNumberOfFiles: 3 } as FileInformationDto)).toBe(true);
  });

  test('Datei exisiert bereits in den Dokumenten', () => {
    const pathToFile = "test/123456/";
    const dokument1 = { filePath: { pathToFile: `${pathToFile}Test1.pdf` } as FilepathDto } as DokumentDto;
    const dokument2 = { filePath: { pathToFile: `${pathToFile}Test2.pdf` } as FilepathDto } as DokumentDto;
  ...
  test('Maximale Größe einer Datei überschritten', () => {
    expect(maxFileSizeExceeded({ size: 100000000 } as File, { maxFileSizeBytes: undefined } as FileInformationDto)).toBe(false);
    expect(maxFileSizeExceeded({ size: 31457281 } as File, { maxFileSizeBytes: 31457280 } as FileInformationDto)).toBe(true);
    expect(maxFileSizeExceeded({ size: 31457280 } as File, { maxFileSizeBytes: 31457280 } as FileInformationDto)).toBe(false);
  ...
  test('Test Allowed MIME-Types', () => {
    let fileInformationDto: FileInformationDto = {
      allowedMimeTypes: ["application/mp4", "application/mxf", "application/vnd.openxmlformats-officedocument.wordprocessingml.template"]
    };
    const result: string = getAllowedMimeTypes(fileInformationDto);
    const expected = "application/mp4,application/mxf,application/vnd.openxmlformats-officedocument.wordprocessingml.template";
